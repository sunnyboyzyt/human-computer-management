...
<html>
  ...
  <body>
	<div class="backRoleSysModel-con">
		...
		<table class="layui-hide" id="Unit" lay-filter="Unit"></table>
	...
	<div id="div_addmajor" style="display: none;text-align: center; ">
		<form class="layui-form" action="">
			<br>
				...
				<div class="layui-form-item">
						<button class="layui-btn" lay-submit lay-filter="submitBut" id="btn_addUnit">立即提交</button>
...
layui.use([ 'table', 'form', 'layer',  'laytpl', 'element' ], function() {
		...
		table.on('tool(Unit)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
		  ...
		  if(layEvent === 'delete'){ //删除
		    layer.confirm('真的删除行么', function(index){
		      ...
		      layer.close(index);
		      //向服务端发送删除指令
		      	$.post("Unit/deletUnit",{unitid:obj.data.unitid},function(flag){
	    			if(flag){
		   ...
		   //脚本编辑弹出层
	             ...
	             layer.open({
		               ...
		               closeBtn:'1',//右上角xx关闭
		               area: ['500px','500px'],
		               content:'./tgls/edit/edit_Unit.jsp',
		               btn: ['确定', '取消'],
		               ...
		               success : function(layero, index){
				            ...
				            if(layEvent === 'edit'){
				                ...
				                body.find("#unitid").val(obj.data.unitid);  //将选中的数据的id传到编辑页面的隐藏域，便于根据ID修改数据
				                body.find("#unitcode").val(obj.data.unitcode); 
				                body.find("#unitname").val(obj.data.unitname);
				                // 记得重新渲染表单
		                		form.render();
				            }
				            setTimeout(function(){
