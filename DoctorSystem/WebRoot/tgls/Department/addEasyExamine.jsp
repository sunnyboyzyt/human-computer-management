...
<html>
  ...
  <body>
	<div class="backRoleSysModel-con">
		...
		<table class="layui-hide" id="EasyExamine" lay-filter="EasyExamine"></table>
	...
	<div id="div_addmajor" style="display: none;text-align: center; ">
		<form class="layui-form" action="">
			...
			<br>
				<div class="layui-form-item">
					...
					<div class="layui-input-inline" style="width:255px;">
						<input type="text" name="easyexamineitem" id="easyexamineitem" required lay-verify="required" autocomplete="off" class="layui-input">
					</div>
				</div>
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-inline" style="width:255px;">
						<input type="text" name="easyexamineparam" id="easyexamineparam" required lay-verify="required" autocomplete="off" class="layui-input">
					</div>
				</div>
			...
			<div class="layui-col-md5">
				...
				<div class="layui-form-item">
						<button class="layui-btn" lay-submit lay-filter="submitBut" id="btn_easyexamine">立即提交</button>
...
layui.use([ 'table', 'form', 'layer',  'laytpl', 'element' ], function() {
		...
		table.render({
			elem : '#EasyExamine',
			id:'EasyExamine',
			url : 'EasyExamine/getEasyExaminelist',
			title : '',
			height : "full-160",
			skin : 'line',
			even : true,
			cols : [ 
			     [{
					...
					field : 'easyexamineitem',
					align : 'center',
					title : '检查项目',
					
				}, {
					field : 'easyexamineparam',
					align : 'center',
					title : '参数',
		...
		table.on('tool(EasyExamine)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
		  ...
		  if(layEvent === 'delete'){ //删除
		    layer.confirm('真的删除行么', function(index){
		      ...
		      layer.close(index);
		      //向服务端发送删除指令
		      	$.post("EasyExamine/deletEasyExamine",{easyexamineid:obj.data.easyexamineid},function(flag){
	    			if(flag){
		   ...
		   //脚本编辑弹出层
	             ...
	             layer.open({
		               ...
		               closeBtn:'1',//右上角xx关闭
		               area: ['500px','500px'],
		               content:'./tgls/edit/edit_EasyExamine.jsp',
		               btn: ['确定', '取消'],
			           ...
			           submit.click();
			           //表格刷新
		          	   table.reload('EasyExamine');
			        },
		               success : function(layero, index){
				            var body = layui.layer.getChildFrame('body', index);
				            if(layEvent === 'edit'){
				                // 取到弹出层里的元素，并把编辑的内容放进去
				                body.find("#easyexamineid").val(obj.data.easyexamineid);  //将选中的数据的id传到编辑页面的隐藏域，便于根据ID修改数据
				                body.find("#easyexamineitem").val(obj.data.easyexamineitem); 
				                body.find("#easyexamineparam").val(obj.data.easyexamineparam);
	...
	//查询结束
	//添加一条方法开始
	$("#btn_easyexamine").click(function(){
			var easyexamineitem=$("#easyexamineitem").val();
			var easyexamineparam=$("#easyexamineparam").val();
			var patientid=$("#patientid").val();
			var obj = {
						"easyexamineitem":easyexamineitem,
						"easyexamineparam":easyexamineparam,
						"patientid":patientid
						};
			$.ajax({
					url : 'EasyExamine/addEasyExamine',
					type : "POST",
					data : obj, //直接传对象参数 
					dataType : 'json',
